...
### TypeScript Responsibility Rules (Mandatory)
- Do not generate monolithic files that combine many `type`, `interface`, constants, helpers, builders, and factories.
- Do not mix DTOs, validators, creation logic, and business logic in the same file.
- Enforce one concept per file and keep folder placement aligned to responsibility.
...
- Avoid loose global helper exports for business concerns.
- Dynamic values must be injected through providers/services. Do not use `Math.random()`, `Date.now()`, or `new Date()` directly in domain or generic helper layers.
- Keep module coupling low and exports minimal.
- Keep event construction logic in dedicated services/factories, separate from domain contracts.
