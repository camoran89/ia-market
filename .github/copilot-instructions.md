...
## MANDATORY: TypeScript Responsibility and Modularity Rules
- These rules are hard constraints for generation and refactoring in TypeScript services and packages.
- Do not create single files that aggregate many `type`, `interface`, constants, helpers, builders, or factories.
- Do not mix DTOs, validators, factories, and business logic in the same file.
- Keep one concept per file and place it in the folder that matches its responsibility.
- Do not create generic accumulator files such as `types.ts`, `models.ts`, `interfaces.ts`, `shared.ts`, `utils.ts`, `helpers.ts`, `factory.ts`.
- Avoid unnecessary global exported functions and global mutable state.
- Do not create loose helper functions like `newCorrelationId()` or `buildAuditEvent()` in generic files.
- Do not call `Math.random()`, `Date.now()`, or `new Date()` directly in domain, DTO, or generic helper files.
- Dynamic values (ids, dates, timestamps, correlation ids) must be produced through injectable providers/services.
- Export only what is required by other modules.
- Keep low coupling and explicit boundaries between modules, domains, and layers.
- Do not mix event creation logic with domain type/interface declarations.
- Avoid utility files that import domain entities only to assemble domain objects.
- Use explicit, responsibility-oriented file names.
- When refactoring, preserve behavior and improve testability and maintainability.
